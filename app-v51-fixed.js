...
function renderCloseStep(){
 ...
 if(closeStep===4){
  ...
  const f=overallFinishStatus();
  $('#closeFinalState').innerHTML=`<b>오늘 마감 준비 완료</b><br>9모 완주 검증: ${f.label} · 보고서를 복사해 보내면 내일 계획을 수정합니다.`
 }
}
...
/* ========================= v5.0 COMPLETE ========================= */
const V5_STORE={locks:'p11122_v50_plan_locks',waiting:'p11122_v50_waiting',automations:'p11122_v50_automations',conflicts:'p11122_v50_automation_conflicts',skips:'p11122_v50_automation_skips',recent:'p11122_v50_recent_learning',bookParts:'p11122_v50_book_part_state'};
function tasksFor(date){const m=allTasks();if(!m[date]){m[date]=[];set('p11122_v2_tasks',m)}return m[date]}
function v5PlanLocks(){return get(V5_STORE.locks,{})}
function isPlanLocked(date){return Boolean(v5PlanLocks()[date])}
function setPlanLocked(date,value){const m=v5PlanLocks();m[date]=Boolean(value);set(V5_STORE.locks,m)}
function waitingItems(){return get(V5_STORE.waiting,[])}
function automationRules(){return get(V5_STORE.automations,[])}
function automationConflicts(){return get(V5_STORE.conflicts,[])}
function automationSkips(){return get(V5_STORE.skips,{})}
function recentLearning(){return get(V5_STORE.recent,[])}
function touchRecentLearning(kind,id,label){let a=recentLearning().filter(x=>!(x.kind===kind&&x.id===id));a.unshift({kind,id,label,at:Date.now()});set(V5_STORE.recent,a.slice(0,8))}
function bookPartState(){return get(V5_STORE.bookParts,{})}
function bookPartKey(x){return `${x.bookId}::${x.mode}::${x.subunit||''}::${x.start||''}-${x.end||''}`}
function bookPartDone(x){return Boolean(bookPartState()[bookPartKey(x)]?.completed)}
function setBookPartDone(x,done){const st=bookPartState();st[bookPartKey(x)]={completed:Boolean(done)};set(V5_STORE.bookParts,st);if(x.mode==='subunit'&&x.subunit)markBookSubunitCompleted(x.bookId,x.subunit,done)}

function normalizeTaskComponents(t){
 if(Array.isArray(t.components)&&t.components.length)return t.components;
 if(Array.isArray(t.lectureIds)&&t.lectureIds.length){t.components=t.lectureIds.map(id=>{const x=lectureInfo(id);return{id:`cmp-${id}`,kind:'lecture',ref:id,label:x?`${String(x.n).padStart(2,'0')}강`:id,done:lectureCompleted(id)}});return t.components}
 if(t.bookItem){const x=t.bookItem;t.components=[{id:`cmp-${bookPartKey(x)}`,kind:'book',bookItem:{...x},label:x.subunit||x.label||t.name,done:x.mode==='subunit'?bookSubunitCompleted(x.bookId,x.subunit):bookPartDone(x)}];return t.components}
 return []
}
function taskComponentState(t){const c=normalizeTaskComponents(t);const done=c.filter(x=>x.done).length;return{components:c,done,total:c.length}}
function taskLinkedBlocks(date,taskId){return timelineForDate(date).filter(b=>(b.taskIds||[]).includes(taskId))}
function syncBlockDoneFromTasks(date){const ts=tasksFor(date),blocks=timelineForDate(date);blocks.forEach(b=>{if(!(b.taskIds||[]).length)return;b.done=b.taskIds.every(id=>ts.find(t=>t.id===id)?.done)});saveTimeline(date,blocks)}
function syncSourceComponent(c,done){c.done=Boolean(done);if(c.kind==='lecture')markLecturesCompleted([c.ref],done);if(c.kind==='book'&&c.bookItem)setBookPartDone(c.bookItem,done)}
function setTaskDoneV5(date,taskId,done,{fromBlock=false}={}){const ts=tasksFor(date),t=ts.find(x=>x.id===taskId);if(!t)return;const comps=normalizeTaskComponents(t);if(comps.length)comps.forEach(c=>syncSourceComponent(c,done));t.done=Boolean(done);saveTasks(date,ts);if(!fromBlock){const blocks=timelineForDate(date);blocks.forEach(b=>{if((b.taskIds||[]).includes(taskId))b.done=Boolean(done)});saveTimeline(date,blocks)}}
function setTaskComponentDone(date,taskId,componentId,done){const ts=tasksFor(date),t=ts.find(x=>x.id===taskId);if(!t)return;const comps=normalizeTaskComponents(t),c=comps.find(x=>x.id===componentId);if(!c)return;syncSourceComponent(c,done);t.done=comps.length?comps.every(x=>x.done):Boolean(done);saveTasks(date,ts);syncBlockDoneFromTasks(date)}
function setBlockDoneV5(date,blockId,done){const blocks=timelineForDate(date),b=blocks.find(x=>x.id===blockId);if(!b)return;b.done=Boolean(done);const ts=tasksFor(date);(b.taskIds||[]).forEach(taskId=>{const linked=blocks.filter(x=>(x.taskIds||[]).includes(taskId)),allDone=Boolean(linked.length)&&linked.every(x=>x.done),t=ts.find(x=>x.id===taskId);if(!t)return;t.done=allDone;if(allDone){normalizeTaskComponents(t).forEach(c=>syncSourceComponent(c,true))}else if(linked.length===1){normalizeTaskComponents(t).forEach(c=>syncSourceComponent(c,false))}});saveTasks(date,ts);saveTimeline(date,blocks)}
function unlinkTaskFromSchedule(date,taskId){const blocks=timelineForDate(date);blocks.forEach(b=>{b.taskIds=(b.taskIds||[]).filter(id=>id!==taskId);if(!b.taskIds.length)b.done=false});saveTimeline(date,blocks)}

function blockTaskHTML(date,b){const ts=tasksFor(date),ids=b.taskIds||[];if(!ids.length){const legacy=genericStudyText(b);return legacy?`<div class="block-task-list"><div class="block-task-item">${esc(legacy)} <span class="block-split">이전 버전 메모</span></div></div>`:'<div class="timeline-study timeline-empty-study">할 일 미배정</div>'}return `<div class="block-task-list">${ids.map(id=>{const t=ts.find(x=>x.id===id);if(!t)return'';const linked=taskLinkedBlocks(date,id),bp=linked.length>1?` · ${linked.filter(x=>x.done).length}/${linked.length}블록`:'';return `<div class="block-task-item ${t.done?'done':''}"><span class="subject">${esc(t.subject)}</span><b>${esc(t.name)}</b><span class="block-split">${bp}</span></div>`}).join('')}</div>`}
function migrateScheduleLinks(date){const ts=tasksFor(date),blocks=timelineForDate(date);let changed=false;blocks.forEach(b=>{if(!Array.isArray(b.taskIds))b.taskIds=[];if(!b.taskIds.length&&Array.isArray(b.lectureIds)&&b.lectureIds.length){const t=ts.find(t=>(t.lectureIds||[]).some(id=>b.lectureIds.includes(id)));if(t){b.taskIds=[t.id];changed=true}}if(!b.taskIds.length){const legacy=genericStudyText(b);if(legacy){const t=ts.find(t=>legacy.includes(t.name)||t.name.includes(legacy));if(t){b.taskIds=[t.id];changed=true}}}});if(changed)saveTimeline(date,blocks)}

function renderTasks(ts){
 const box=$('#todayTasks');box.innerHTML='';
 if(!ts.length){box.innerHTML='<div class="empty-action-state"><b>오늘 할 일이 아직 없습니다.</b><span>학습 자판기에서 담거나 `+ 직접 추가`로 만든 뒤 시간표에서 선택하세요.</span></div>';return}
 ts.forEach(t=>{const st=taskComponentState(t),linked=taskLinkedBlocks(selected,t.id),partial=st.total?`${st.done}/${st.total} 완료`:linked.length>1?`${linked.filter(b=>b.done).length}/${linked.length} 블록`:'';const e=document.createElement('div');e.className='task'+(t.done?' done':'');e.innerHTML=`<div class="task-main"><input class="task-main-check" type="checkbox" ${t.done?'checked':''}><div><div><span class="priority ${t.priority}">${PRIORITY_LABEL[t.priority]}</span><span class="subject">${esc(t.subject)}</span><span class="task-title">${esc(t.name)}</span></div><div class="task-meta">${esc(t.material||'')}${t.duration?' · '+esc(t.duration):''}${t.note?' · '+esc(t.note):''}</div>${partial?`<div class="task-progress-line"><span class="partial-badge">${partial}</span>${linked.length?`<span class="task-meta">시간표 ${linked.length}곳 배정</span>`:''}</div>`:''}${st.total>1?`<details class="task-components"><summary>세부 완료 펼치기</summary><div class="task-component-list">${st.components.map(c=>`<label class="component-check ${c.done?'done':''}"><input class="component-toggle" data-cid="${esc(c.id)}" type="checkbox" ${c.done?'checked':''}>${esc(c.label)}</label>`).join('')}</div></details>`:''}</div><div class="task-actions"><button class="btn ghost small edit">수정</button><button class="btn ghost small wait">대기함</button><button class="btn danger small del">삭제</button></div></div>`;e.querySelector('.task-main-check').onchange=x=>{setTaskDoneV5(selected,t.id,x.target.checked);renderDashboard();if($('#lectures')?.classList.contains('active'))renderLearningVending();renderCourses()};e.querySelectorAll('.component-toggle').forEach(ch=>ch.onchange=()=>{setTaskComponentDone(selected,t.id,ch.dataset.cid,ch.checked);renderDashboard();renderCourses();if($('#lectures')?.classList.contains('active'))renderLearningVending()});e.querySelector('.edit').onclick=()=>openTask(t);e.querySelector('.wait').onclick=()=>moveTaskToWaiting(selected,t.id);e.querySelector('.del').onclick=()=>{if(confirm('삭제할까요?')){unlinkTaskFromSchedule(selected,t.id);saveTasks(selected,ts.filter(x=>x.id!==t.id));renderDashboard()}};box.appendChild(e)})
}

function renderSchedule(slots){migrateScheduleLinks(selected);slots=timelineForDate(selected);const box=$('#todaySchedule');box.innerHTML=slots.length?'':'<div class="note">시간표가 없습니다.</div>';slots.forEach(s=>{const e=document.createElement('div');e.className='slot'+(s.done?' done':'')+(s.locked?' locked':'');e.innerHTML=`<div class="time">${esc(s.time||'')}</div><div class="school">${s.locked?'🔒 ':''}${esc(s.name||s.school||'')}</div><div class="study">${s.selfStudy?blockTaskHTML(selected,s):esc(s.study||'')}</div><div class="slot-actions">${s.selfStudy?'<button class="btn ghost small slot-edit-study">할 일 선택</button>':''}<label><input class="block-done-check" type="checkbox" ${s.done?'checked':''}> 완료</label></div>`;e.querySelector('.block-done-check').onchange=x=>{setBlockDoneV5(selected,s.id,x.target.checked);renderDashboard()};const edit=e.querySelector('.slot-edit-study');if(edit)edit.onclick=()=>openAssignTaskModal(selected,s.id);box.appendChild(e)})}

let assigningBlock={date:'',id:''};
function openAssignTaskModal(date,blockId){assigningBlock={date,id:blockId};const blocks=timelineForDate(date),b=blocks.find(x=>x.id===blockId),ts=tasksFor(date);$('#assignTaskDate').value=date;$('#assignTaskBlockId').value=blockId;$('#assignTaskBlockLabel').textContent=`${b?.time||''} · ${b?.name||b?.school||''}`;$('#assignTaskSearch').value='';renderAssignTaskList();$('#assignTaskModal').classList.add('show')}
function renderAssignTaskList(){const date=$('#assignTaskDate').value,id=$('#assignTaskBlockId').value,b=timelineForDate(date).find(x=>x.id===id),q=($('#assignTaskSearch').value||'').trim().toLowerCase(),ts=tasksFor(date).filter(t=>!q||`${t.subject} ${t.name} ${t.material}`.toLowerCase().includes(q));const selectedIds=new Set(b?.taskIds||[]);$('#assignTaskList').innerHTML=ts.length?ts.map(t=>{const used=taskLinkedBlocks(date,t.id).length;return `<label class="assign-task-option ${used?'used':''}"><input class="assign-task-check" data-id="${t.id}" type="checkbox" ${selectedIds.has(t.id)?'checked':''}><div><b>${esc(t.subject)} · ${esc(t.name)}</b><span>${esc(t.duration||'시간 미입력')} · ${t.done?'완료':'미완료'}</span></div><span>${used?`다른/현재 블록 ${used}곳`:'미배정'}</span></label>`}).join(''):'<div class="empty-action-state"><b>선택할 오늘 할 일이 없습니다.</b><span>오늘 화면의 `+ 직접 추가` 또는 학습 자판기에서 먼저 추가하세요. 시간표에서는 이름을 직접 입력하지 않습니다.</span></div>';updateAssignCount();$$('.assign-task-check').forEach(x=>x.onchange=updateAssignCount)}
function updateAssignCount(){if(!$('#assignTaskCount'))return;$('#assignTaskCount').textContent=`${$$('.assign-task-check:checked').length}개 선택`}
$('#assignTaskSearch')?.addEventListener('input',renderAssignTaskList);
$('#saveBlockTasks')?.addEventListener('click',()=>{const date=$('#assignTaskDate').value,id=$('#assignTaskBlockId').value,blocks=timelineForDate(date),b=blocks.find(x=>x.id===id);if(!b)return;b.taskIds=$$('.assign-task-check:checked').map(x=>x.dataset.id);b.study='';b.done=b.taskIds.length?b.taskIds.every(tid=>tasksFor(date).find(t=>t.id===tid)?.done):false;saveTimeline(date,blocks);$('#assignTaskModal').classList.remove('show');renderTimelineEditor();if(date===selected)renderDashboard()});
$('#clearBlockTasks')?.addEventListener('click',()=>{$$('.assign-task-check').forEach(x=>x.checked=false);updateAssignCount()});

function renderTimelineSummary(date,blocks){if(!$('#availableStudySummary'))return;const self=blocks.filter(x=>x.selfStudy),regular=self.filter(x=>x.regular).length,known=self.reduce((s,x)=>s+timeMinutes(x.start,x.end),0),device=self.filter(x=>x.device).length,assigned=self.filter(x=>(x.taskIds||[]).length).length;$('#availableStudySummary').innerHTML=`<div class="timeline-stat"><span>정규 자습</span><b>${regular}교시</b></div><div class="timeline-stat"><span>시각 확정 자습</span><b>${known}분</b></div><div class="timeline-stat"><span>전자기기 가능</span><b>${device}블록</b></div><div class="timeline-stat"><span>할 일 배정</span><b>${assigned}/${self.length}</b></div>`}
function renderTimelineEditor(){if(!$('#dailyTimelineEditor'))return;const date=$('#timelineDate').value||selected;runAutomationForDate(date);migrateScheduleLinks(date);const blocks=timelineForDate(date);$('#timelineDate').value=date;renderTimelineSummary(date,blocks);renderPlanLockButtons(date);$('#dailyTimelineEditor').innerHTML=blocks.length?blocks.map(b=>`<div class="timeline-block ${b.locked?'locked':''} ${b.done?'done':''}"><div class="timeline-time">${esc(b.time||b.name)}</div><div><div class="timeline-name">${b.locked?'🔒 ':''}${esc(b.name||b.school||'시간 블록')}</div>${b.selfStudy?blockTaskHTML(date,b):`<div class="timeline-study">${esc(b.study||'')}</div>`}<div class="timeline-tags">${b.selfStudy?'<span class="school-tag self">자습</span>':''}${b.selfStudy?(b.device?'<span class="school-tag device">전자기기</span>':'<span class="school-tag paper">종이</span>'):''}</div></div><div class="timeline-actions">${b.selfStudy?`<button class="btn ghost small assign-block" data-id="${b.id}">할 일 선택</button>`:''}<button class="btn ghost small edit-block" data-id="${b.id}">블록 설정</button><label><input class="timeline-done" data-id="${b.id}" type="checkbox" ${b.done?'checked':''}> 완료</label></div></div>`).join(''):'<div class="note">기본 시간표가 없습니다. 집 공부나 시간 블록을 추가할 수 있습니다.</div>';$$('.assign-block').forEach(btn=>btn.onclick=()=>openAssignTaskModal(date,btn.dataset.id));$$('.edit-block').forEach(btn=>btn.onclick=()=>openTimeBlockModal(date,blocks.find(x=>x.id===btn.dataset.id)));$$('.timeline-done').forEach(ch=>ch.onchange=()=>{setBlockDoneV5(date,ch.dataset.id,ch.checked);renderTimelineEditor();if(date===selected)renderDashboard()})}

function renderPlanLockButtons(date=selected){const locked=isPlanLocked(date),label=locked?'🔒 계획 확정됨':'오늘 계획 확정';if($('#togglePlanLock')){$('#togglePlanLock').textContent=label;$('#togglePlanLock').classList.toggle('plan-locked',locked)}if($('#timelinePlanLock')){$('#timelinePlanLock').textContent=locked?'🔒 계획 확정됨':'계획 확정';$('#timelinePlanLock').classList.toggle('plan-locked',locked)}if($('#todayPlanState')){$('#todayPlanState').textContent=locked?'🔒 계획 확정':'계획 열림';$('#todayPlanState').className='badge'+(locked?' plan-locked':'')}}
function toggleDatePlanLock(date){setPlanLocked(date,!isPlanLocked(date));renderPlanLockButtons(date);if(date===selected)renderDashboard()}
$('#togglePlanLock')?.addEventListener('click',()=>toggleDatePlanLock(selected));
$('#timelinePlanLock')?.addEventListener('click',()=>toggleDatePlanLock($('#timelineDate').value||selected));

function renderTodayFocus(date=selected){
 const ts=tasksFor(date),done=ts.filter(t=>t.done).length;
 $('#focusTaskCount').textContent=ts.length?`${done}/${ts.length}`:'0개';
 const next=ts.find(t=>!t.done);$('#focusNextTask').textContent=next?next.name:(ts.length?'오늘 할 일 완료':'아직 없음');
 let current='해당 없음';const blocks=timelineForDate(date),today=date===ymd(now());
 if(today){const n=now(),mins=n.getHours()*60+n.getMinutes();const b=blocks.find(x=>x.start&&x.end&&mins>=Number(x.start.slice(0,2))*60+Number(x.start.slice(3))&&mins<Number(x.end.slice(0,2))*60+Number(x.end.slice(3)));if(b)current=b.name||b.school}
 $('#focusCurrentBlock').textContent=current;
 const remain=blocks.filter(b=>b.selfStudy&&!b.done).reduce((s,b)=>s+timeMinutes(b.start,b.end),0),untimed=blocks.filter(b=>b.selfStudy&&!b.done&&!b.start).length;
 $('#focusRemainTime').textContent=(remain||untimed)?`${Math.floor(remain/60)}h ${remain%60}m${untimed?` + ${untimed}교시`:''}`:'0분';
 renderPlanLockButtons(date);const conflicts=automationConflicts().filter(c=>c.date===date),banner=$('#automationConflictBanner');if(conflicts.length){banner.classList.remove('hidden');banner.textContent=`반복 할 일 미완료 충돌 ${conflicts.length}건 · 눌러서 처리`;banner.onclick=()=>navigate('automation')}else banner.classList.add('hidden')
}
function renderDashboard(){runAutomationForDate(selected);migrateScheduleLinks(selected);const ts=tasksFor(selected),slots=scheduleFor(selected),done=ts.filter(t=>t.done).length,rate=ts.length?Math.round(done/ts.length*100):0;$('#phaseBadge').textContent=phase(selected);const info=INITIAL_WEEK[selected],st=settings(),mode=info?.mode||(parse(selected).getDay()===0?'일요일 회복':'학기 모드');$('#modeText').textContent=mode;const goalOverrides=get('p11122_v23_goals',{});$('#goalHours').textContent=(goalOverrides[selected]??info?.goal??(parse(selected).getDay()===0?4.5:st.schoolGoal)).toFixed(1);$('#todayRate').textContent=rate+'%';$('#todayBar').style.width=rate+'%';const h=get('p11122_v2_hours',{});$('#hoursInput').value=h[selected]||'';const[ws,we]=weekBounds(parse(selected));let wh=0;for(let d=new Date(ws);d<=we;d.setDate(d.getDate()+1))wh+=Number(h[ymd(d)]||0);$('#weekStudy').textContent=wh.toFixed(1);renderTodayFocus(selected);renderTasks(ts);renderSchedule(slots);renderConditionQuick();renderFeasibilityQuick()}

/* Waiting list */
function moveTaskToWaiting(date,id){const ts=tasksFor(date),t=ts.find(x=>x.id===id);if(!t)return;if(!confirm('이 할 일을 이번 주 대기함으로 옮길까요?'))return;unlinkTaskFromSchedule(date,id);const w=waitingItems();w.push({...t,waitingSince:date,done:false});set(V5_STORE.waiting,w);saveTasks(date,ts.filter(x=>x.id!==id));renderDashboard();renderWaitingList()}
function renderWaitingList(){if(!$('#waitingList'))return;const w=waitingItems();$('#waitingList').innerHTML=w.length?w.map(t=>`<div class="waiting-item"><div><b>${esc(t.subject)} · ${esc(t.name)}</b><span>${esc(t.material||'')} · ${esc(t.duration||'시간 미입력')} · ${t.waitingSince?`대기 ${t.waitingSince}`:''}</span></div><div class="row"><button class="btn primary small waiting-today" data-id="${t.id}">오늘로 보내기</button><button class="btn danger small waiting-delete" data-id="${t.id}">삭제</button></div></div>`).join(''):'<div class="note">대기 중인 할 일이 없습니다.</div>';$$('.waiting-today').forEach(b=>b.onclick=()=>{const list=waitingItems(),t=list.find(x=>x.id===b.dataset.id);if(!t)return;const date=selected,ts=tasksFor(date);if(ts.some(x=>x.id===t.id))t.id=uid();delete t.waitingSince;t.done=false;ts.push(t);saveTasks(date,ts);set(V5_STORE.waiting,list.filter(x=>x!==t&&x.id!==b.dataset.id));renderWaitingList();renderDashboard()});$$('.waiting-delete').forEach(b=>b.onclick=()=>{set(V5_STORE.waiting,waitingItems().filter(x=>x.id!==b.dataset.id));renderWaitingList()})}
$('#addWaitingTask')?.addEventListener('click',()=>{$('#waitingName').value='';$('#waitingMaterial').value='';$('#waitingDuration').value='';$('#waitingTaskModal').classList.add('show')});
$('#saveWaitingTask')?.addEventListener('click',()=>{const name=$('#waitingName').value.trim();if(!name){alert('할 일을 입력해 주세요.');return}const w=waitingItems();w.push({id:uid(),subject:$('#waitingSubject').value,priority:'should',name,material:$('#waitingMaterial').value.trim(),duration:$('#waitingDuration').value.trim(),note:'',done:false,waitingSince:selected});set(V5_STORE.waiting,w);$('#waitingTaskModal').classList.remove('show');renderWaitingList()});

/* Learning vending = single source of progress */
function learningProgressItems(){const rows=lectureCourses().map(c=>({kind:'인강',sourceKind:'lecture',id:c.key,subject:c.subject,name:c.display,provider:c.provider,done:lectureCourseDone(c),total:c.total,note:`${c.provider} · ${c.series}`}));problemBooks().forEach(b=>{const total=(b.subunits||[]).length,done=(b.subunits||[]).filter(s=>bookSubunitCompleted(b.id,s)).length;rows.push({kind:'문제집',sourceKind:'book',id:b.id,subject:b.subject,name:b.name,provider:'',done,total,note:total?`소단원 ${done}/${total}`:'소단원 미등록'})});return rows}
function courseData(){const d={kor:[],eco:[],soc:[],other:[]};learningProgressItems().forEach(x=>{const meta=COURSE_META[x.name]||{},c={name:x.name,done:x.done,total:x.total,note:`${x.kind} · ${x.note}`,deadline:meta.deadline||'2026-09-01',dailyTarget:meta.dailyTarget||1,sourceKind:x.sourceKind};if(x.subject==='국어')d.kor.push(c);else if(x.subject==='경제')d.eco.push(c);else if(x.subject==='사회문화')d.soc.push(c);else d.other.push(c)});return d}
function syncCourseProgressFromLectures(){}
function setCourseProgressDone(){}
function renderCourses(){if(!$('#courseProgressCatalog'))return;const rows=learningProgressItems(),subjects=[...new Set(rows.map(x=>x.subject))];$('#courseProgressCatalog').innerHTML=subjects.length?subjects.map(s=>`<section class="progress-subject-group"><h4>${esc(s)}</h4><div class="progress-card-grid">${rows.filter(x=>x.subject===s).map(x=>{const pct=x.total?Math.round(x.done/x.total*100):0;return `<div class="learning-progress-card"><span class="kind">${x.kind}</span><h5>${esc(x.name)}</h5><div class="task-meta">${esc(x.note)}</div><div class="progress-line"><i style="width:${pct}%"></i></div><b>${x.total?`${x.done}/${x.total} · ${pct}%`:`완료 기준을 소단원으로 등록해 주세요.`}</b></div>`}).join('')}</div></section>`).join(''):'<div class="note">학습 자판기에 등록된 강좌나 문제집이 없습니다.</div>';renderFeasibility()}

/* Problem-book grouping */
function leadingNumber(s){const m=String(s||'').match(/^\s*0*(\d+)/);return m?Number(m[1]):null}
function compactSubunitRange(items,book){const subs=book?.subunits||[],names=items.map(x=>x.subunit);if(names.length===1)return names[0];const nums=names.map(leadingNumber);if(nums.every(n=>n!==null)&&nums.every((n,i)=>i===0||n===nums[i-1]+1))return `${String(nums[0]).padStart(2,'0')}~${String(nums.at(-1)).padStart(2,'0')}`;return `${names[0]}~${names.at(-1)}`}
function groupBookCartItems(items){const out=[];const books=problemBooks();const byBook={};items.forEach(x=>(byBook[x.bookId]||(byBook[x.bookId]=[])).push(x));Object.entries(byBook).forEach(([bookId,list])=>{const book=books.find(b=>b.id===bookId);const problems=list.filter(x=>x.mode==='problems').sort((a,b)=>(a.subunit||'').localeCompare(b.subunit||'')||a.start-b.start);problems.forEach(x=>{let g=out.at(-1);if(g&&g.mode==='problems'&&g.bookId===bookId&&g.subunit===(x.subunit||'')&&Number(x.start)<=Number(g.end)+1){g.items.push(x);g.end=Math.max(g.end,x.end);g.minutes+=Number(x.minutes||0)}else out.push({mode:'problems',bookId,book,subunit:x.subunit||'',start:x.start,end:x.end,items:[x],minutes:Number(x.minutes||0)})});const subitems=list.filter(x=>x.mode==='subunit').sort((a,b)=>(book?.subunits||[]).indexOf(a.subunit)-(book?.subunits||[]).indexOf(b.subunit));subitems.forEach(x=>{const idx=(book?.subunits||[]).indexOf(x.subunit),g=out.at(-1),lastIdx=g?.indices?.at(-1);if(g&&g.mode==='subunit'&&g.bookId===bookId&&idx===lastIdx+1){g.items.push(x);g.indices.push(idx);g.minutes+=Number(x.minutes||0)}else out.push({mode:'subunit',bookId,book,items:[x],indices:[idx],minutes:Number(x.minutes||0)})})});return out}
function bookGroupTask(g){const book=g.book||{name:g.items[0]?.bookName||'문제집',subject:g.items[0]?.subject||'기타'},components=g.items.map(x=>({id:`cmp-${bookPartKey(x)}`,kind:'book',bookItem:{...x},label:x.mode==='subunit'?x.subunit:`${x.start}~${x.end}번`,done:x.mode==='subunit'?bookSubunitCompleted(x.bookId,x.subunit):bookPartDone(x)}));const name=g.mode==='problems'?`${book.name}${g.subunit?' · '+g.subunit:''} ${g.start}~${g.end}번`:`${book.name} · ${compactSubunitRange(g.items,book)}`;return{id:uid(),subject:book.subject,priority:'must',name,material:book.name,note:`학습 자판기 · 문제집 · ${g.mode==='problems'?'문제 수 기준':'소단원 기준'}`,duration:`${g.minutes||30}분`,done:components.length?components.every(c=>c.done):false,components,bookComponents:g.items.map(x=>({...x})),taskKind:'book'}}
function autoPlaceTaskIds(date,newTasks){if(isPlanLocked(date))return 0;const blocks=timelineForDate(date),available=blocks.filter(b=>b.selfStudy&&!b.locked&&!(b.taskIds||[]).length);let placed=0;const device=available.filter(b=>b.device),paper=available.filter(b=>!b.device);newTasks.filter(t=>t.taskKind==='lecture').forEach(t=>{const b=device.find(x=>!(x.taskIds||[]).length);if(b){b.taskIds=[t.id];b.study='';placed++}});newTasks.filter(t=>t.taskKind==='book').forEach(t=>{const b=paper.find(x=>!(x.taskIds||[]).length)||device.find(x=>!(x.taskIds||[]).length);if(b){b.taskIds=[t.id];b.study='';placed++}});saveTimeline(date,blocks);return placed}
function buildLecturePlan(){const date=$('#lecturePlanDate').value||selected,lcart=lectureCart().filter(id=>!lectureCompleted(id)),bcart=bookCart();if(!lcart.length&&!bcart.length){alert('장바구니에 학습 항목을 먼저 담아 주세요.');return}const ts=tasksFor(date),existingIds=new Set(ts.flatMap(t=>t.lectureIds||[])),fresh=lcart.filter(id=>!existingIds.has(id)),newTasks=[];groupLectureIds(fresh).forEach(g=>{const c=g.course,components=g.ids.map(id=>{const x=lectureInfo(id);return{id:`cmp-${id}`,kind:'lecture',ref:id,label:`${String(x.n).padStart(2,'0')}강`,done:lectureCompleted(id)}});newTasks.push({id:uid(),subject:c.subject,priority:'must',name:`${c.display} ${rangeLabel(g.numbers)}`,material:`${c.provider} · ${c.series}`,note:'학습 자판기 · 인강',duration:`${g.ids.length}강`,done:false,lectureIds:g.ids,components,taskKind:'lecture'})});groupBookCartItems(bcart).forEach(g=>newTasks.push(bookGroupTask(g)));ts.push(...newTasks);saveTasks(date,ts);const st=lectureState();fresh.forEach(id=>st[id]={...(st[id]||{}),completed:false,plannedDate:date});set('p11122_v30_lecture_state',st);newTasks.forEach(t=>{if(t.taskKind==='lecture'){const x=lectureInfo(t.lectureIds[0]);if(x)touchRecentLearning('lecture',x.key,x.display)}else if(t.taskKind==='book'&&t.bookComponents?.[0]){const b=problemBooks().find(b=>b.id===t.bookComponents[0].bookId);if(b)touchRecentLearning('book',b.id,b.name)}});const placed=autoPlaceTaskIds(date,newTasks);saveLectureCart([]);saveBookCart([]);selected=date;renderDashboard();renderWeek();renderLearningVending();renderCourses();if($('#timelineDate'))$('#timelineDate').value=date;renderTimelineEditor();navigate('dashboard');alert(`${date}에 ${newTasks.length}개 묶음 할 일을 추가했습니다.\n시간표 자동배치 ${placed}개${newTasks.length>placed?' · 나머지는 할 일 목록에 유지':''}`)}
function toggleLectureCartId(id){if(lectureCompleted(id))return;const cart=lectureCart(),i=cart.indexOf(id),x=lectureInfo(id);if(i>=0)cart.splice(i,1);else{cart.push(id);if(x)touchRecentLearning('lecture',x.key,x.display)}saveLectureCart(cart);renderLearningVending()}

function toggleBookSubunitCart(book,subunit){const cart=bookCart(),i=cart.findIndex(x=>x.bookId===book.id&&x.mode==='subunit'&&x.subunit===subunit);if(i>=0)cart.splice(i,1);else{cart.push({id:uid(),kind:'book',mode:'subunit',bookId:book.id,subject:book.subject,bookName:book.name,subunit,minutes:30,label:`${book.name} · ${subunit}`});touchRecentLearning('book',book.id,book.name)}saveBookCart(cart);renderLearningVending()}
function renderProblemBooks(){if(!$('#problemBookCatalog'))return;const list=problemBooks(),cart=bookCart();$('#problemBookCatalog').innerHTML=list.length?list.map(book=>`<section class="book-card"><div class="book-card-head"><div><h4>${esc(book.subject)} · ${esc(book.name)}</h4><div class="book-meta">소단원 ${(book.subunits||[]).length}개 · 소단원은 클릭클릭 장바구니 / 문제 수는 범위 입력</div></div><div class="book-actions"><button class="btn primary small book-problem-task" data-id="${book.id}">문제 수로 담기</button><button class="btn ghost small edit-book" data-id="${book.id}">편집</button><button class="btn danger small delete-book" data-id="${book.id}">삭제</button></div></div><div class="subunit-grid">${(book.subunits||[]).length?(book.subunits||[]).map(sub=>{const inCart=cart.some(x=>x.bookId===book.id&&x.mode==='subunit'&&x.subunit===sub),done=bookSubunitCompleted(book.id,sub);return `<button class="subunit-chip ${done?'completed':''} ${inCart?'cart':''}" data-book="${book.id}" data-subunit="${esc(sub)}"><b>${esc(sub)}</b><span>${inCart?'장바구니 ✓':done?'완료 · 다시 담을 수 있음':'클릭해서 담기'}</span></button>`}).join(''):'<div class="note">소단원이 없습니다. 편집에서 한 줄에 하나씩 추가하세요.</div>'}</div></section>`).join(''):'<div class="note">등록된 문제집이 없습니다. `+ 문제집 등록`에서 시작하세요.</div>';$$('.book-problem-task').forEach(b=>b.onclick=()=>{const book=list.find(x=>x.id===b.dataset.id);if(book)openBookTaskModal(book,'problems')});$$('.subunit-chip').forEach(b=>b.onclick=()=>{const book=list.find(x=>x.id===b.dataset.book);if(book)toggleBookSubunitCart(book,b.dataset.subunit)});$$('.edit-book').forEach(b=>b.onclick=()=>{const book=list.find(x=>x.id===b.dataset.id);if(book)openProblemBookModal(book)});$$('.delete-book').forEach(b=>b.onclick=()=>{const book=list.find(x=>x.id===b.dataset.id);if(!book||!confirm(`${book.name}을 삭제할까요?`))return;saveProblemBooks(list.filter(x=>x.id!==book.id));saveBookCart(bookCart().filter(x=>x.bookId!==book.id));renderLearningVending();renderCourses()})}
function renderRecentLearning(){if(!$('#recentLearning'))return;const a=recentLearning();$('#recentLearning').innerHTML=a.length?a.map(x=>`<button class="recent-chip" data-kind="${x.kind}" data-id="${x.id}">${esc(x.label)}</button>`).join(''):'<span class="task-meta">아직 최근 사용 항목이 없습니다.</span>';$$('.recent-chip').forEach(b=>b.onclick=()=>{if(b.dataset.kind==='lecture'){$('#lectureSubjectFilter').value='all';fillLectureCourseFilter();$('#lectureCourseFilter').value=b.dataset.id;currentLearningTab='lecture'}else{const book=problemBooks().find(x=>x.id===b.dataset.id);$('#learningSearch').value=book?.name||'';currentLearningTab='book'}renderLearningVending()})}
function applyLearningSearch(){const q=($('#learningSearch')?.value||'').trim().toLowerCase();$$('.lecture-course').forEach(el=>el.classList.toggle('learning-search-hidden',q&&!el.textContent.toLowerCase().includes(q)));$$('.book-card').forEach(el=>el.classList.toggle('learning-search-hidden',q&&!el.textContent.toLowerCase().includes(q)))}
function renderLearningVending(){renderLearningTabs();renderLectures();renderProblemBooks();renderCombinedLearningCart();renderRecentLearning();applyLearningSearch();const date=$('#lecturePlanDate').value||selected;$('#lecturePlanDate').value=date;const blocks=timelineForDate(date),open=blocks.filter(b=>b.selfStudy&&!b.locked),device=open.filter(b=>b.device).length,paper=open.filter(b=>!b.device).length;$('#lectureCartSchoolHint').innerHTML=`${DAY_NAMES[parse(date).getDay()]} · 사용 가능한 자습 ${open.length}블록 · 전자기기 ${device} · 종이 ${paper} · 장바구니 ${combinedCartCount()}개${combinedCartCount()>open.length?'<br><span class="learning-overflow">블록보다 할 일이 많아 초과분은 오늘 할 일에만 추가됩니다.</span>':''}`}
$('#learningSearch')?.addEventListener('input',applyLearningSearch);$('#clearLearningSearch')?.addEventListener('click',()=>{$('#learningSearch').value='';applyLearningSearch()});
$('#addBookTaskToCart')?.addEventListener('click',()=>{setTimeout(()=>{const last=bookCart().at(-1);if(last){const b=problemBooks().find(x=>x.id===last.bookId);if(b)touchRecentLearning('book',b.id,b.name)}renderRecentLearning()},0)});

/* Automation */
function automationSourceOptions(){return [...lectureCourses().map(c=>({value:`lecture:${c.key}`,kind:'lecture',id:c.key,subject:c.subject,label:`인강 · ${c.provider} ${c.display}`})),...problemBooks().map(b=>({value:`book:${b.id}`,kind:'book',id:b.id,subject:b.subject,label:`문제집 · ${b.subject} ${b.name}`}))]}
function sourceOption(value){return automationSourceOptions().find(x=>x.value===value)}
function nextAutomationTask(rule,date){const src=sourceOption(rule.source);if(!src)return null;if(src.kind==='lecture'){const c=lectureCourse(src.id);if(!c)return null;let id='';for(let n=1;n<=c.total;n++){const lid=lectureId(c.key,n);if(!lectureCompleted(lid)&&!lecturePlannedDate(lid)){id=lid;break}}if(!id)return null;const x=lectureInfo(id);return{id:uid(),subject:c.subject,priority:rule.priority||'must',name:`${c.display} ${x.n}강`,material:`${c.provider} · ${c.series}`,duration:rule.duration||'1강',note:'반복 자동화 · 다음 미수강 강의',done:false,lectureIds:[id],components:[{id:`cmp-${id}`,kind:'lecture',ref:id,label:`${String(x.n).padStart(2,'0')}강`,done:false}],taskKind:'lecture',automationRuleId:rule.id,autoKey:`auto50-${rule.id}-${date}`}}const b=problemBooks().find(x=>x.id===src.id);if(!b)return null;const planned=new Set(Object.values(allTasks()).flat().filter(t=>!t.done).flatMap(t=>(t.bookComponents||[]).filter(x=>x.bookId===b.id&&x.mode==='subunit').map(x=>x.subunit)));const sub=(b.subunits||[]).find(s=>!bookSubunitCompleted(b.id,s)&&!planned.has(s));if(sub){const item={id:uid(),kind:'book',mode:'subunit',bookId:b.id,subject:b.subject,bookName:b.name,subunit:sub,minutes:Number(String(rule.duration||'30').match(/\d+/)?.[0]||30),label:`${b.name} · ${sub}`};const t=bookGroupTask({mode:'subunit',bookId:b.id,book:b,items:[item],indices:[(b.subunits||[]).indexOf(sub)],minutes:item.minutes});return{...t,priority:rule.priority||'must',duration:rule.duration||t.duration,note:'반복 자동화 · 다음 미완료 소단원',automationRuleId:rule.id,autoKey:`auto50-${rule.id}-${date}`}}return{id:uid(),subject:b.subject,priority:rule.priority||'must',name:b.name,material:b.name,duration:rule.duration||'30분',note:'반복 자동화 · 문제집',done:false,taskKind:'book',automationRuleId:rule.id,autoKey:`auto50-${rule.id}-${date}`}}
function ruleApplies(rule,date){const d=parse(date),day=d.getDay();return rule.enabled!==false&&(rule.weekdays||[]).map(Number).includes(day)&&(!rule.start||date>=rule.start)&&(!rule.end||date<=rule.end)&&!automationSkips()[`${rule.id}:${date}`]}
function runAutomationForDate(date){if(isPlanLocked(date))return;const rules=automationRules(),m=allTasks();if(!m[date])m[date]=[];let changed=false,conf=automationConflicts();rules.forEach(rule=>{if(!ruleApplies(rule,date))return;if(m[date].some(t=>t.autoKey===`auto50-${rule.id}-${date}`))return;if(conf.some(c=>c.ruleId===rule.id&&c.date===date))return;const previous=[];Object.keys(m).filter(d=>d<date).sort().forEach(d=>(m[d]||[]).filter(t=>t.automationRuleId===rule.id&&!t.done).forEach(t=>previous.push({date:d,task:t})));const proposal=nextAutomationTask(rule,date);if(!proposal)return;if(previous.length){const p=previous.at(-1);conf.push({id:uid(),ruleId:rule.id,date,previousDate:p.date,previousTaskId:p.task.id,proposal});set(V5_STORE.conflicts,conf);return}m[date].push(proposal);if(proposal.lectureIds){const st=lectureState();proposal.lectureIds.forEach(id=>st[id]={...(st[id]||{}),plannedDate:date,completed:false});set('p11122_v30_lecture_state',st)}changed=true});if(changed)set('p11122_v2_tasks',m)}
function openAutomationRule(rule=null){const opts=automationSourceOptions();if(!opts.length){alert('학습 자판기에 강좌나 문제집을 먼저 등록해 주세요.');return}$('#automationSource').innerHTML=opts.map(x=>`<option value="${x.value}">${esc(x.label)}</option>`).join('');$('#automationRuleId').value=rule?.id||'';$('#automationSource').value=rule?.source||opts[0].value;$$('#automationWeekdays input').forEach(x=>x.checked=(rule?.weekdays||[1,2,3,4,5,6]).map(Number).includes(Number(x.value)));$('#automationStart').value=rule?.start||ymd(now());$('#automationEnd').value=rule?.end||'';$('#automationPriority').value=rule?.priority||'must';$('#automationDuration').value=rule?.duration||'';$('#automationEnabled').checked=rule?.enabled!==false;$('#automationRuleModal').classList.add('show')}
$('#addAutomationRule')?.addEventListener('click',()=>openAutomationRule());
$('#saveAutomationRule')?.addEventListener('click',()=>{const weekdays=$$('#automationWeekdays input:checked').map(x=>Number(x.value));if(!weekdays.length){alert('반복 요일을 하나 이상 선택해 주세요.');return}const rules=automationRules(),id=$('#automationRuleId').value||uid(),obj={id,source:$('#automationSource').value,weekdays,start:$('#automationStart').value,end:$('#automationEnd').value,priority:$('#automationPriority').value,duration:$('#automationDuration').value.trim(),enabled:$('#automationEnabled').checked},i=rules.findIndex(x=>x.id===id);if(i>=0)rules[i]=obj;else rules.push(obj);set(V5_STORE.automations,rules);$('#automationRuleModal').classList.remove('show');runAutomationForDate(selected);renderAutomation();renderDashboard()});
function weekdayText(days){const n={0:'일',1:'월',2:'화',3:'수',4:'목',5:'금',6:'토'};return(days||[]).map(x=>n[x]).join('·')}
function skipRuleToday(ruleId,date=selected){const skips=automationSkips();skips[`${ruleId}:${date}`]=true;set(V5_STORE.skips,skips);const m=allTasks();if(m[date])m[date]=m[date].filter(t=>!(t.automationRuleId===ruleId&&!t.done));set('p11122_v2_tasks',m);set(V5_STORE.conflicts,automationConflicts().filter(c=>!(c.ruleId===ruleId&&c.date===date)));renderAutomation();renderDashboard()}
function resolveAutomationConflict(id,action){let conflicts=automationConflicts(),c=conflicts.find(x=>x.id===id);if(!c)return;const m=allTasks(),prev=m[c.previousDate]?.find(t=>t.id===c.previousTaskId),proposal=c.proposal;if(action==='merge'){if(prev){m[c.previousDate]=m[c.previousDate].filter(t=>t.id!==prev.id);unlinkTaskFromSchedule(c.previousDate,prev.id);prev.autoKey=proposal.autoKey;prev.automationRuleId=c.ruleId;prev.note=(prev.note||'')+' · 전날 미완료와 오늘분 합침';(m[c.date]||(m[c.date]=[])).push(prev)}}else if(action==='separate'){(m[c.date]||(m[c.date]=[])).push(proposal)}else if(action==='skipPrevious'){if(prev){m[c.previousDate]=m[c.previousDate].filter(t=>t.id!==prev.id);unlinkTaskFromSchedule(c.previousDate,prev.id)}(m[c.date]||(m[c.date]=[])).push(proposal)}set('p11122_v2_tasks',m);conflicts=conflicts.filter(x=>x.id!==id);set(V5_STORE.conflicts,conflicts);renderAutomation();if(c.date===selected)renderDashboard()}
function renderAutomation(){if(!$('#automationRuleList'))return;const rules=automationRules(),opts=automationSourceOptions();$('#automationRuleList').innerHTML=rules.length?rules.map(r=>{const s=opts.find(x=>x.value===r.source);return `<div class="automation-rule ${r.enabled===false?'paused':''}"><div class="automation-rule-top"><div><b>${esc(s?.label||'삭제된 학습 항목')}</b><div class="weekday-badges"><span class="weekday-badge">${weekdayText(r.weekdays)}</span>${r.start?`<span class="weekday-badge">${r.start}~${r.end||'계속'}</span>`:''}</div><div class="task-meta">${PRIORITY_LABEL[r.priority||'must']} · ${esc(r.duration||'시간 미입력')} · ${r.enabled===false?'일시정지':'활성'}</div></div><div class="row"><button class="btn ghost small auto-edit" data-id="${r.id}">수정</button><button class="btn ghost small auto-skip" data-id="${r.id}">오늘만 건너뛰기</button><button class="btn warn small auto-toggle" data-id="${r.id}">${r.enabled===false?'재개':'일시정지'}</button><button class="btn danger small auto-delete" data-id="${r.id}">삭제</button></div></div></div>`}).join(''):'<div class="note">반복 규칙이 없습니다.</div>';$$('.auto-edit').forEach(b=>b.onclick=()=>openAutomationRule(rules.find(x=>x.id===b.dataset.id)));$$('.auto-skip').forEach(b=>b.onclick=()=>skipRuleToday(b.dataset.id));$$('.auto-toggle').forEach(b=>b.onclick=()=>{const a=automationRules(),r=a.find(x=>x.id===b.dataset.id);if(r)r.enabled=r.enabled===false;set(V5_STORE.automations,a);renderAutomation()});$$('.auto-delete').forEach(b=>b.onclick=()=>{if(!confirm('이 반복 규칙을 삭제할까요?'))return;set(V5_STORE.automations,automationRules().filter(x=>x.id!==b.dataset.id));set(V5_STORE.conflicts,automationConflicts().filter(x=>x.ruleId!==b.dataset.id));renderAutomation()});const conf=automationConflicts();$('#automationConflictCount').textContent=`${conf.length}건`;$('#automationConflictList').innerHTML=conf.length?conf.map(c=>{const rule=rules.find(r=>r.id===c.ruleId),s=opts.find(x=>x.value===rule?.source);return `<div class="automation-conflict"><b>${esc(s?.label||'반복 할 일')}</b><div class="task-meta">${c.previousDate} 미완료 → ${c.date} 새 반복일</div><div class="automation-conflict-actions"><button class="btn primary small conflict-action" data-id="${c.id}" data-action="merge">오늘 것과 합치기</button><button class="btn ghost small conflict-action" data-id="${c.id}" data-action="separate">별도 유지</button><button class="btn warn small conflict-action" data-id="${c.id}" data-action="skipPrevious">이전 것은 건너뛰기</button></div></div>`}).join(''):'<div class="note">미완료 충돌이 없습니다.</div>';$$('.conflict-action').forEach(b=>b.onclick=()=>resolveAutomationConflict(b.dataset.id,b.dataset.action))}

/* Recent 7-day minimal stats */
function renderRecent7Stats(){if(!$('#recent7Stats'))return;const end=parse(selected),start=new Date(end);start.setDate(start.getDate()-6);const tm=allTasks(),hm=get('p11122_v2_hours',{}),counts={};SUBJECTS.forEach(s=>counts[s]=0);let total=0,done=0,hours=0;for(let d=new Date(start);d<=end;d.setDate(d.getDate()+1)){const ds=ymd(d);hours+=Number(hm[ds]||0);(tm[ds]||[]).forEach(t=>{total++;if(t.done){done++;if(counts[t.subject]!=null)counts[t.subject]++}})}const vals=Object.entries(counts),max=Math.max(0,...vals.map(x=>x[1])),min=Math.min(...vals.map(x=>x[1])),most=vals.sort((a,b)=>b[1]-a[1])[0],least=vals.sort((a,b)=>a[1]-b[1])[0],balance=max-min<=2?'균형 양호':`${most[0]} 쏠림 / ${least[0]} 부족`;$('#recent7Stats').innerHTML=`<div class="recent7-stat"><span>완료율</span><b>${total?Math.round(done/total*100):0}%</b></div><div class="recent7-stat"><span>순공</span><b>${hours.toFixed(1)}h</b></div><div class="recent7-stat"><span>완료 할 일</span><b>${done}/${total}</b></div><div class="recent7-stat"><span>과목 편중</span><b>${max-min}</b></div><div class="subject-balance-row">최근 7일 과목 완료: ${SUBJECTS.map(s=>`${s} ${counts[s]}`).join(' · ')}<br><b>${balance}</b></div>`}

/* Close-day completion uses same task/schedule source */
function renderCloseTasks(){const ts=tasksFor(closeDate);$('#closeTaskList').innerHTML=ts.length?ts.map(t=>`<label class="close-task"><input class="close-task-check" data-id="${t.id}" type="checkbox" ${t.done?'checked':''}><div><b><span class="priority ${t.priority}">${PRIORITY_LABEL[t.priority]}</span>${esc(t.subject)} · ${esc(t.name)}</b><span>${esc(t.duration||'시간 미입력')}${t.material?' · '+esc(t.material):''}</span></div></label>`).join(''):'<div class="note">등록된 과제가 없습니다.</div>';$$('.close-task-check').forEach(x=>x.onchange=()=>{setTaskDoneV5(closeDate,x.dataset.id,x.checked);renderDashboard()})}

function renderReport(){const[ws,we]=weekBounds(parse(selected)),tm=allTasks(),hm=get('p11122_v2_hours',{});let hours=0,done=0,total=0,sub={};SUBJECTS.forEach(s=>sub[s]={done:0,total:0});for(let d=new Date(ws);d<=we;d.setDate(d.getDate()+1)){const ds=ymd(d);hours+=Number(hm[ds]||0);(tm[ds]||[]).forEach(t=>{total++;if(t.done)done++;if(sub[t.subject]){sub[t.subject].total++;if(t.done)sub[t.subject].done++}})}const rate=total?Math.round(done/total*100):0;$('#repHours').textContent=hours.toFixed(1);$('#repRate').textContent=rate+'%';$('#repTests').textContent=tests().filter(t=>parse(t.date)>=ws&&parse(t.date)<=we).length;$('#repHike').textContent=settings().hikeEnabled?'ON':'OFF';$('#subjectReport').innerHTML=SUBJECTS.map(s=>`<tr><td>${s}</td><td>${sub[s].done}</td><td>${sub[s].total}</td><td>${sub[s].total?Math.round(sub[s].done/sub[s].total*100):0}%</td></tr>`).join('');const weak=SUBJECTS.map(s=>[s,sub[s].total?sub[s].done/sub[s].total:1]).sort((a,b)=>a[1]-b[1])[0][0];$('#managerReport').innerHTML=`<div class="report-block"><b>총평</b>이번 주 순공 ${hours.toFixed(1)}시간, 계획 달성률 ${rate}%입니다. ${rate>=90?'현재 계획 강도를 유지합니다.':rate>=75?'필수 과제는 유지하고 권장 과제를 일부 줄입니다.':'총량을 줄이고 필수 과제부터 복구해야 합니다.'}</div><div class="report-block"><b>가장 위험한 과목</b>${weak}의 완료율이 가장 낮습니다. 다음 주에는 새로운 자료보다 미완료 핵심을 우선 배치합니다.</div><div class="report-block"><b>이월 규칙</b>필수만 다음 가능한 날로 이동하고, 권장은 대기함·주말 후보, 여유는 삭제합니다.</div>`;renderRecent7Stats()}

function renderWeek(){const d=parse(selected),[ws,we]=weekBounds(d),days=[];for(let x=new Date(ws);x<=we;x.setDate(x.getDate()+1))days.push(ymd(x));$('#calendarGrid').innerHTML=days.map(x=>`<button class="day ${x===selected?'active':''}" data-date="${x}"><b>${parse(x).getMonth()+1}/${parse(x).getDate()}</b><span>${['일','월','화','수','목','금','토'][parse(x).getDay()]}</span><small>${tasksFor(x).filter(t=>t.done).length}/${tasksFor(x).length}</small></button>`).join('');$$('#calendarGrid .day').forEach(b=>b.onclick=()=>{selected=b.dataset.date;$('#selectedDate').value=selected;renderWeek();updateSelectedSummary()});$('#selectedDate').value=selected;updateSelectedSummary();$('#weekTable').innerHTML=days.map(date=>{const ts=tasksFor(date);return `<tr><td>${date}</td>${SUBJECTS.map(s=>`<td>${ts.filter(t=>t.subject===s).map(t=>`${t.done?'✓ ':'○ '}${esc(t.name)}`).join('<br>')||'-'}</td>`).join('')}</tr>`}).join('');renderWaitingList()}

function navigate(page){$$('.section').forEach(s=>s.classList.toggle('active',s.id===page));$$('nav button').forEach(b=>b.classList.toggle('active',b.dataset.page===page));const nav=$(`nav button[data-page="${page}"]`);$('#pageTitle').textContent=nav?nav.textContent:(page==='data'?'설정·백업':'PROJECT 11122');if(page==='manager')renderManager();if(page==='condition')renderCondition();if(page==='week')renderWeek();if(page==='school')renderSchool();if(page==='lectures')renderLearningVending();if(page==='automation')renderAutomation();if(page==='courses')renderCourses();if(page==='tests')renderTests();if(page==='report')renderReport();if(page==='hike')renderHike()}

function migrateV50(){if(get('p11122_v50_migrated',false))return;const tm=allTasks();Object.keys(tm).forEach(date=>{(tm[date]||[]).forEach(t=>normalizeTaskComponents(t))});set('p11122_v2_tasks',tm);const schedules=allSchedules();Object.keys(schedules).forEach(date=>{(schedules[date]||[]).forEach(b=>{if(!Array.isArray(b.taskIds))b.taskIds=[]})});set('p11122_v2_schedules',schedules);set('p11122_v50_migrated',true)}

migrateV21();migrateV23();migrateV30();migrateV40();migrateV50();runAutomationForDate(selected);if($('#timelineDate'))$('#timelineDate').value=selected;renderDashboard();renderCondition();renderManager();renderWeek();renderSchool();renderLearningVending();renderCourses();renderTests();renderReport();renderHike();renderAutomation();renderWaitingList();
if($('#runtimeBuild'))$('#runtimeBuild').textContent='JS 5.1 연결됨';
if('serviceWorker'in navigator)window.addEventListener('load',()=>navigator.serviceWorker.register('./sw-v51.js').catch(()=>{}));
