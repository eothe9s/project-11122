...
function renderPlanner(){
 ...
 $('#plannerModeHint').textContent=plannerSleepMode?'☾ 취침 시각 선택':(plannerEdit?'편집 · 탭/드래그':'보기');
...
function renderPlannerQuickPanel(){
 ...
 <div class="planner-sleep-box">
   <div class="planner-sleep-row"><span>☾ 취침</span><b>${meta.bed||'–'}</b></div>
   <div class="planner-sleep-row"><span>☼ 기상</span><b>${meta.wake||'–'}</b></div>
   <div class="planner-sleep-row"><span>수면</span><b>${sleepSpanLabel(meta.bed,meta.wake)}</b></div>
 </div>`
}
...
function init(){
 ...
 if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw-v62.js?v=620').catch(()=>{});
}
