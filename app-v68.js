...
function normalizeDBShape(d){
 ...
 ['customLectures','books','automations','automationConflicts','waiting','tests','recentLearning','trash','closeHistory'].forEach(k=>{if(!Array.isArray(d[k]))d[k]=[]});
 ['tasks','schedules','lectureState','bookState','automationSkips','automationRuns','condition','plannerMeta','studyOverrides','planLocks','meta'].forEach(k=>{if(!d[k]||typeof d[k]!=='object'||Array.isArray(d[k]))d[k]={}});
...
function baseSchedule(date){
 ...
 if(day>=1&&day<=5){
  a.push({id:`${date}-morning`,schema:SCHEMA_VERSION,fixed:true,name:'아침 자습',type:'self',selfStudy:true,device:false,start:'07:50',end:'08:30',taskIds:[],locked:false,done:false,actualMin:null});
  (SCHOOL[day]||[]).forEach((r,i)=>{
   if(day===5&&i+1===5)return;
   const p=i+1,pt=DB.settings.periodTimes?.[p]||{};
   a.push({id:`${date}-p${p}`,schema:SCHEMA_VERSION,fixed:true,regular:true,period:p,name:r[0],type:r[1]?'self':'class',selfStudy:Boolean(r[1]),device:Boolean(r[2]),start:pt.start||'',end:pt.end||'',taskIds:[],locked:false,done:false,actualMin:null})
  });
  a.push({id:`${date}-lunch`,schema:SCHEMA_VERSION,fixed:true,name:'점심시간',type:'meal',selfStudy:false,device:false,start:'12:30',end:'13:00',taskIds:[],locked:true,done:false,actualMin:null});
  if(day===1||day===3)a.push({id:`${date}-lunch-program`,schema:SCHEMA_VERSION,fixed:true,name:'영단어 프로그램',type:'class',selfStudy:false,device:false,start:'13:00',end:'13:30',taskIds:[],locked:true,done:false,actualMin:null});
  if(day===2||day===4)a.push({id:`${date}-lunch-program`,schema:SCHEMA_VERSION,fixed:true,name:'국어 프로그램',type:'class',selfStudy:false,device:false,start:'13:00',end:'13:30',taskIds:[],locked:true,done:false,actualMin:null});
  if(day===5){const p5end=DB.settings.periodTimes?.[5]?.end||'';a.push({id:`${date}-english-mock`,schema:SCHEMA_VERSION,fixed:true,name:'영어 모의고사',type:'class',selfStudy:false,device:false,start:'13:00',end:p5end,taskIds:[],locked:true,done:false,actualMin:null})}
  const after=day<=3?['방과후 자습','self',true,true]:day===4?['나혜주 선생님 영어 독해','class',false,false]:['문두열 선생님 수학Ⅰ·수학Ⅱ·미적분(상)','class',false,false];
  a.push({id:`${date}-after`,schema:SCHEMA_VERSION,fixed:true,name:after[0],type:after[1],selfStudy:after[2],device:after[3],start:'16:45',end:'17:45',taskIds:[],locked:!after[2],done:false,actualMin:null});
  a.push({id:`${date}-dinner`,schema:SCHEMA_VERSION,fixed:true,name:'석식',type:'meal',selfStudy:false,device:false,start:'17:45',end:'18:40',taskIds:[],locked:true,done:false,actualMin:null});
  a.push({id:`${date}-night1`,schema:SCHEMA_VERSION,fixed:true,name:'야간자율학습 1',type:'self',selfStudy:true,device:true,start:'18:40',end:'20:20',taskIds:[],locked:false,done:false,actualMin:null});
  a.push({id:`${date}-break`,schema:SCHEMA_VERSION,fixed:true,name:'쉬는 시간',type:'break',selfStudy:false,device:false,start:'20:20',end:'20:30',taskIds:[],locked:true,done:false,actualMin:null});
  a.push({id:`${date}-night2`,schema:SCHEMA_VERSION,fixed:true,name:'야간자율학습 2',type:'self',selfStudy:true,device:true,start:'20:30',end:'22:00',taskIds:[],locked:false,done:false,actualMin:null});
 } else if(day===6){
  SATURDAY.forEach((r,i)=>a.push({id:`${date}-sat${i+1}`,schema:SCHEMA_VERSION,fixed:true,name:r[2],type:'self',selfStudy:true,device:true,start:r[0],end:r[1],taskIds:[],locked:false,done:false,actualMin:null}))
 }
 return a
}
function normalizeBlock(b,date){
 const x={...deep(b),id:b.id||uid(),schema:SCHEMA_VERSION,fixed:Boolean(b.fixed),regular:Boolean(b.regular),period:b.period||null,name:b.name||b.school||'시간 블록',type:b.type||(b.selfStudy?'self':'custom'),selfStudy:Boolean(b.selfStudy),device:Boolean(b.device),start:b.start||'',end:b.end||'',taskIds:Array.isArray(b.taskIds)?b.taskIds:[],locked:Boolean(b.locked),done:Boolean(b.done),actualMin:b.actualMin==null?null:Number(b.actualMin),userOverrides:b.userOverrides?deep(b.userOverrides):null};
...
function renderWeek(){
 ...
 let html='<table class="week-table"><thead><tr><th>구간</th>'+dates.map((d,i)=>`<th>${['월','화','수','목','금'][i]}<small>${Number(d.slice(-2))}일</small></th>`).join('')+'</tr></thead><tbody>';
