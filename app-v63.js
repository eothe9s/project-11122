/* PROJECT 11122 v6.3 · CSAT EDITION */
'use strict';
...
function setTaskComponentDone(date,id,cid,done){
 ...
 syncBlocksFromTasks(date);saveDB()
}
function taskLinkedBlocks(date,taskId){return ensureSchedule(date).filter(b=>(b.taskIds||[]).includes(taskId))}
function syncBlocksFromTasks(date){
 const blocks=ensureSchedule(date);
 blocks.forEach(b=>{
  if(!(b.taskIds||[]).length)return;
  b.done=b.taskIds.every(id=>taskById(date,id)?.done)
 });
 DB.schedules[date]=blocks
}
function setBlockDone(date,blockId,done){
 const blocks=ensureSchedule(date),b=blocks.find(x=>x.id===blockId);if(!b)return;
 b.done=Boolean(done);DB.schedules[date]=blocks;
 (b.taskIds||[]).forEach(id=>{
  const links=blocks.filter(x=>(x.taskIds||[]).includes(id));
  const allDone=links.length?links.every(x=>x.done):Boolean(done);
  // 블록 완료는 분할 배정을 보존한다. 일부 블록만 끝났다면 그 블록의 완료 표시는 유지하고,
  // 연결된 모든 블록이 끝났을 때만 할 일 전체를 완료한다.
  setTaskDoneInternal(date,id,allDone,true)
 });
 saveDB()
}
...
function mergeSchedule(date,old){
 ...
 base.forEach(n=>{
  ...
  if(o){used.add(o.id);const q=normalizeBlock(o,date);n.taskIds=q.taskIds;n.done=q.done;n.actualMin=q.actualMin;n.locked=n.selfStudy?q.locked:n.locked}
 });
...
function renderDashboard(){
 ...
 const tasks=tasksFor(selected),done=tasks.filter(t=>t.done).length,stage=activeStage(selected),pb=plannedStudy(selected),auto=autoActualStudy(selected),final=finalStudy(selected);
 ...
 $('#focusTaskCount').textContent=`${done}/${tasks.length}`;
...
function renderTaskList(){
 const box=$('#todayTasks'),list=tasksFor(selected);
 if(!list.length){box.innerHTML='<div class="muted">아직 할 일이 없습니다. 학습 자판기나 자동화에서 추가하거나 직접 추가하세요.</div>';return}
 box.innerHTML=list.map(t=>{
  const st=taskState(t),linked=taskLinkedBlocks(selected,t.id),partial=st.total>1?`${st.done}/${st.total} 완료`:linked.length>1?`${linked.filter(b=>b.done).length}/${linked.length} 블록`:'';
  return `<div class="task ${t.done?'done':''}" data-task="${t.id}">
   <div class="task-row">
    ...
    <div>
      ...
      ${st.total>1?`<details class="component-toggle"><summary>세부 완료</summary><div class="component-list">${t.components.map(c=>`<label class="component-item"><input class="component-check" data-tid="${t.id}" data-cid="${c.id}" type="checkbox" ${c.done?'checked':''}>${esc(c.label)}</label>`).join('')}</div></details>`:''}
    </div>
 ...
 $$('.component-check').forEach(x=>x.onchange=()=>{setTaskComponentDone(selected,x.dataset.tid,x.dataset.cid,x.checked);renderDashboard();renderProgress();renderVendingIfVisible()});
...
function renderPlanner(){
 ...
 $('#unknownRegularStrip').innerHTML=unknown.length?`<div class="muted" style="width:100%">교시 시각 미설정 · ⚙에서 입력</div>`+unknown.map(b=>`<span class="regular-chip ${b.selfStudy?'self':''}">${b.period}교시 ${esc(b.name)}${b.selfStudy?` · 자습${b.device?'·기기':'·종이'}`:''}</span>`).join(''):'';
 $('#plannerLegend').innerHTML=plannerLegendHTML();
 renderPlannerQuickPanel();
 renderTenGrid()
}
function renderPlannerQuickPanel(){
 const meta=planMeta(selected),cond=DB.condition?.[selected]||{},next=nextSelfStudyBlock(selected),current=currentBlock(selected),unfinished=tasksFor(selected).filter(t=>!t.done).length;
 const wake=cond.wake||meta.wake||'';
 $('#plannerQuickPanel').innerHTML=`<div class="planner-quick-grid">
   <div class="planner-quick-item"><span>현재</span><b>${current?esc(current.name):'없음'}</b></div>
 ...
 <div class="planner-sleep-box">
   <div class="planner-sleep-row"><div class="planner-sleep-icon">☾</div><div class="planner-sleep-value">${meta.bed||'–'}</div></div>
   <div class="planner-sleep-row"><div class="planner-sleep-icon">☀</div><div class="planner-sleep-value">${wake||'–'}</div></div>
   <div class="planner-sleep-row"><div class="planner-sleep-icon">◔</div><div class="planner-sleep-value">${sleepSpanLabel(meta.bed,wake)}</div></div>
 </div>`
}
function renderTenGrid(){
 const cond=DB.condition?.[selected]||{};
 const box=$('#tenMinutePlanner'),blocks=ensureSchedule(selected),mapped=new Map(),meta=planMeta(selected),sleepIdx=meta.bed?plannerIndexForTime(meta.bed):null,wakeVal=cond.wake||meta.wake||'',wakeIdx=wakeVal?plannerIndexForTime(wakeVal):null;
 ...
 for(let h=5;h<25;h++){
  ...
  for(let k=0;k<6;k++){
   ...
   if(isSleep&&isWake)label=`<span class="cell-label">☾☀</span>`;
   else if(isSleep)label=`<span class="cell-label">☾</span>`;
   else if(isWake)label=`<span class="cell-label">☀</span>`;
...
function openSleepModal(date,time=''){
 const meta=planMeta(date);$('#sleepDate').value=date;$('#sleepBed').value=time||meta.bed||'00:30';$('#sleepWake').value=meta.wake||'06:30';$('#sleepSummaryText').textContent=`수면 ${sleepSpanLabel($('#sleepBed').value,$('#sleepWake').value)}`;showModal('sleepModal')
}
...
function saveBlockModal(){
 ...
 b.name=$('#blockName').value.trim()||'시간 블록';b.type=$('#blockType').value;b.start=$('#blockStart').value;b.end=$('#blockEnd').value;b.selfStudy=$('#blockSelfStudy').checked;b.device=b.selfStudy&&$('#blockDevice').checked;b.locked=$('#blockLocked').checked;b.actualMin=$('#blockActual').value===''?null:Number($('#blockActual').value);
...
function saveAssignments(){
 const date=$('#assignDate').value,id=$('#assignBlockId').value,blocks=ensureSchedule(date),b=blocks.find(x=>x.id===id);if(!b)return;b.taskIds=$$('.assign-check:checked').map(x=>x.value);b.done=b.taskIds.length&&b.taskIds.every(tid=>taskById(date,tid)?.done);saveSchedule(date,blocks);hideModal('assignModal');renderPlanner();renderDashboard()
}
...
function calcSleep(bed,wake){if(!bed||!wake)return'';let a=timeToMin(bed),b=timeToMin(wake);if(b<=a)b+=1440;const m=b-a;return `${Math.floor(m/60)}시간 ${m%60}분`}
function renderCondition(){
 $('#conditionDate').value=$('#conditionDate').value||selected;const d=$('#conditionDate').value,c=DB.condition[d]||{};
 $('#bedTime').value=c.bed||'';$('#wakeTime').value=c.wake||'';$('#sleepTotal').value=calcSleep(c.bed,c.wake);$('#sleepQuality').value=c.quality||'';$('#fatigue').value=c.fatigue??'';$('#headache').value=c.headache??'';$('#focusScore').value=c.focus??'';$('#expectedCondition').value=c.expected||'';$('#eveningFatigue').value=c.eveningFatigue??'';$('#dailyFocus').value=c.dailyFocus??'';$('#overallCondition').value=c.overall||'';$('#caffeineCups').value=c.caffeine??'';$('#lastCaffeine').value=c.lastCaffeine||'';$('#vitaminTaken').checked=Boolean(c.vitamin);$('#conditionMemo').value=c.memo||'';$$('.symptomCheck').forEach(x=>x.checked=(c.symptoms||[]).includes(x.value))
}
function saveCondition(){
 const d=$('#conditionDate').value;DB.condition[d]={bed:$('#bedTime').value,wake:$('#wakeTime').value,quality:$('#sleepQuality').value,fatigue:$('#fatigue').value,headache:$('#headache').value,focus:$('#focusScore').value,expected:$('#expectedCondition').value,eveningFatigue:$('#eveningFatigue').value,dailyFocus:$('#dailyFocus').value,overall:$('#overallCondition').value,caffeine:Number($('#caffeineCups').value)||0,lastCaffeine:$('#lastCaffeine').value,vitamin:$('#vitaminTaken').checked,symptoms:$$('.symptomCheck:checked').map(x=>x.value),memo:$('#conditionMemo').value.trim()};saveDB();renderCondition();if(d===selected)renderDashboard()
}
...
function renderVersionStatus(){$('#runtimeStatus').textContent='6.2 · SW 6.2'}
...
function bindEvents(){
 ...
 $('#conditionDate').onchange=renderCondition;$('#bedTime').onchange=()=>$('#sleepTotal').value=calcSleep($('#bedTime').value,$('#wakeTime').value);$('#wakeTime').onchange=()=>$('#sleepTotal').value=calcSleep($('#bedTime').value,$('#wakeTime').value);$('#saveCondition').onclick=saveCondition;$('#analysisMonth').onchange=renderAnalysis;$('#hikeEnabled').onchange=()=>{DB.settings.hikeEnabled=$('#hikeEnabled').checked;saveDB()};
...
function init(){
 ...
 if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw-v63.js?v=630').catch(()=>{});
}
