...
vm.createContext(ctx);vm.runInContext(fs.readFileSync(__dirname+'/app-v68.js','utf8'),ctx);
...
test('schema 6 migrates to 8 and infers automation run',()=>{
 exec(`const d=todayDate(),old={...defaultDB(),schema:6};old.tasks[d]=[normalizeImportedTask({id:'a',subject:'경제',name:'CORE',automationRuleId:'r',components:[{id:'c',kind:'manual',label:'x',done:false}]})];const m=migrateDB(old);globalThis.out={schema:m.schema,run:m.automationRuns[automationRunKey('r',d)]?.status}`);assert.deepStrictEqual(plain(val('out')),{schema:8,run:'task'});
});
