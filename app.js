...
function renderTodayFocus(date=selected){const ts=tasksFor(date),done=ts.filter(t=>t.done).length;$('#focusTaskCount').textContent=`${done}/${ts.length}`;const next=ts.find(t=>!t.done);$('#focusNextTask').textContent=next?next.name:'오늘 할 일 완료';let current='-';const blocks=timelineForDate(date),today=date===ymd(now());if(today){const n=now(),mins=n.getHours()*60+n.getMinutes();const b=blocks.find(x=>x.start&&x.end&&mins>=Number(x.start.slice(0,2))*60+Number(x.start.slice(3))&&mins<Number(x.end.slice(0,2))*60+Number(x.end.slice(3)));if(b)current=b.name||b.school}$('#focusCurrentBlock').textContent=current;const remain=blocks.filter(b=>b.selfStudy&&!b.done).reduce((s,b)=>s+timeMinutes(b.start,b.end),0),untimed=blocks.filter(b=>b.selfStudy&&!b.done&&!b.start).length;$('#focusRemainTime').textContent=`${Math.floor(remain/60)}h ${remain%60}m${untimed?` + ${untimed}교시`:''}`;renderPlanLockButtons(date);const conflicts=automationConflicts().filter(c=>c.date===date);const banner=$('#automationConflictBanner');if(conflicts.length){banner.classList.remove('hidden');banner.textContent=`반복 할 일 미완료 충돌 ${conflicts.length}건 · 눌러서 처리`;banner.onclick=()=>navigate('automation')}else banner.classList.add('hidden')}
function renderDashboard(){runAutomationForDate(selected);migrateScheduleLinks(selected);const ts=tasksFor(selected),slots=scheduleFor(selected),done=ts.filter(t=>t.done).length,rate=ts.length?Math.round(done/ts.length*100):0;$('#phaseBadge').textContent=phase(selected);const info=INITIAL_WEEK[selected],st=settings(),mode=info?.mode||(parse(selected).getDay()===0?'일요일 회복':'학기 모드');$('#modeText').textContent=mode;const goalOverrides=get('p11122_v23_goals',{});$('#goalHours').textContent=(goalOverrides[selected]??info?.goal??(parse(selected).getDay()===0?4.5:st.schoolGoal)).toFixed(1);$('#todayRate').textContent=rate+'%';$('#todayBar').style.width=rate+'%';const h=get('p11122_v2_hours',{});$('#hoursInput').value=h[selected]||'';const[ws,we]=weekBounds(parse(selected));let wh=0;for(let d=new Date(ws);d<=we;d.setDate(d.getDate()+1))wh+=Number(h[ymd(d)]||0);$('#weekStudy').textContent=wh.toFixed(1);renderTodayFocus(selected);renderTasks(ts);renderSchedule(slots);renderConditionQuick();renderFeasibilityQuick();renderMinimumSuccess()}
...
function renderCloseTasks(){const ts=tasksFor(closeDate);$('#closeTaskList').innerHTML=ts.length?ts.map(t=>`<label class="close-task"><input class="close-task-check" data-id="${t.id}" type="checkbox" ${t.done?'checked':''}><div><b><span class="priority ${t.priority}">${PRIORITY_LABEL[t.priority]}</span>${esc(t.subject)} · ${esc(t.name)}</b><span>${esc(t.duration||'시간 미입력')}${t.material?' · '+esc(t.material):''}</span></div></label>`).join(''):'<div class="note">등록된 과제가 없습니다.</div>';$$('.close-task-check').forEach(x=>x.onchange=()=>{setTaskDoneV5(closeDate,x.dataset.id,x.checked);renderCloseMinimum();renderDashboard()});renderCloseMinimum()}
